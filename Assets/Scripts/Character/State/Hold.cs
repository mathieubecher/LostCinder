...
public class Hold : State
{
    ...
    public override void Update()
    {
        ...
        if(character.pushCinder != null && Math.Abs(character.pushCinder.GetComponent<Rigidbody2D>().velocity.x) < MovementPush(character.speed)*0.8f) {
            ...
            if (Math.Abs(velocityCinder.x ) < Math.Abs(MovementPush(character.movement * character.speed))*0.5f) {
                velocityCinder.x += MovementPush(character.movement * character.speed)/ 3f;
